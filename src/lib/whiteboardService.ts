...
): Promise<{ room: WhiteboardRoom | null; error: any }> {
  try {
...
export async function joinWhiteboardRoom(
  ...
  try {
    ...
    if (participants && participants.length >= room.max_participants) {
      return { room: null, error: new Error('Room is full') };
    }
...
export async function getRoomParticipants(
  roomId: string
): Promise<{ participants: WhiteboardParticipant[]; error: any }> {
  try {
...
export async function getDrawingHistory(
  roomId: string
): Promise<{ events: DrawingEvent[]; error: any }> {
  try {
    const { data, error } = await supabase
      .from('whiteboard_events')
      .select('*')
